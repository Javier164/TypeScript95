...
class Win95 {
	...
	OEM() {
		...
		let dividend: number | string | null = Math.floor((Math.random() * 9_999_999) + 1_000_000); // Underscored numbers for readability.
	...
	Test() {
		...
		for (let i = 0; i < 25; i++) {
			console.log(`Windows 95 OEM Key ${i}: ${Windows.OEM()}`); // Prints a bunch of OEM keys to test.
		}
	}
}
